...
function buildAdminLayer(data){
    return L.geoJSON(data, {
        ...
        onEachFeature: function(feature, layer){
            ...
            const luas  = p.luas != null ? Number(p.luas).toLocaleString('id-ID', {maximumFractionDigits:2}) + ' Ha' : '-';
            const c = DESA_COLORS[desa] || { fill:'#f6c667', border:'#c97d11' };
            layer.bindPopup(`
    ...
    if(props){
        const rows = Object.entries(props).map(([k,v]) => {
            ...
            if(isLuas && val !== '-' && !isNaN(Number(val))){
                const ha = (Number(val) / 10000).toFixed(4);
                val = `${Number(val).toLocaleString('id-ID')} m² <span style="color:#7a90a4;font-size:10px;">(${ha} Ha)</span>`;
            }
