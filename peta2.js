...
function buildBidangLayer(data){
    return L.geoJSON(data, {
        ...
        onEachFeature: function(feature, layer){
            ...
            const stat = p.STATUS || '-';
            const zona = p.ZONA   || '-';
            const _luasM2  = p.LUAS != null ? Number(p.LUAS) : null;
            const _luasHa  = _luasM2 != null ? _luasM2 / 10000 : null;
            const _luasKm2 = _luasM2 != null ? _luasM2 / 1000000 : null;
            const luas = _luasM2 != null
                ? _luasM2.toLocaleString('id-ID', {maximumFractionDigits:2}) + ' m²'
                  + ' <span style="color:#888;font-size:11px;">('
                  + _luasHa.toLocaleString('id-ID', {minimumFractionDigits:4, maximumFractionDigits:4}) + ' Ha'
                  + ')</span>'
                  + ' <span style="color:#888;font-size:11px;">('
                  + _luasKm2.toLocaleString('id-ID', {minimumFractionDigits:4, maximumFractionDigits:4}) + ' km²'
                  + ')</span>'
                : '-';
            const badgeMap={
    ...
    container.innerHTML = activeKeys.map(name => {
        ...
        <div class="legend-section" id="legend-upload-${CSS.escape(name)}" style="margin-top:10px;">
            ...
            <div class="${gridClass}" style="margin-bottom:6px;">
                ...
                <div class="legend-item wrap-text">
                    <div class="legend-dot" style="background:${colorMap[v] || '#888'};border:1px solid rgba(0,0,0,0.15);flex-shrink:0;margin-top:2px;"></div>
                    <span title="${v}">${v}</span>
                </div>`).join('')}
