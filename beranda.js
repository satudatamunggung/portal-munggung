...
function initVisitorStats() {
    ...
    const KEY_ONLINE  = 'sip_online_ts';   // timestamp heartbeat sesi ini
    const KEY_ONLINE_ALL = 'sip_online_all'; // array semua heartbeat aktif
    ...
    // Reset harian jika hari beda
    if (savedDate !== todayStr) {
        today = 0;
        localStorage.setItem(KEY_DATE, todayStr);
    }

    // Hitung hanya sekali per session (pakai sessionStorage)
    ...
    // ── Heartbeat online (simpan timestamp tiap 10 detik) ──
    function heartbeat() {
        const now = Date.now();
        let all = [];
        try { all = JSON.parse(localStorage.getItem(KEY_ONLINE_ALL) || '[]'); } catch {}
        // Simpan ID sesi unik per tab
        ...
        // Update/tambah entry ini
        all = all.filter(e => e.id !== tabId && (now - e.ts) < 30000);
        all.push({ id: tabId, ts: now });
